...
const baseConversion = function(src, dest, size, format) {
  format = format || "";

  const baseDir = 'app/images/';
  var imgResizeOpt = { width: size };
...
[375,750,660,1320].forEach(function(size){
  ...
  gulp.task(taskName, function() {
    baseConversion('.__raw/to_process/*.{jpeg,jpg,png}', size, size)
  });
  resizeImageTasks.push(taskName);

  // WEBP Conversion __________________________________________

  taskName = 'resize_webp_' + size;
  gulp.task(taskName, function() {
    baseConversion('.__raw/to_process/*.{jpeg,jpg,png}', size, size, 'webp')
  });
  resizeImageTasks.push(taskName);

});

gulp.task('process-img', resizeImageTasks);

gulp.task('min-img', () => {
  return gulp.src('app/images/.__raw/*')
    .pipe(imagemin({
      progressive: true,
